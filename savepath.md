...
├─ tags → tags (array)
├─ category → category_id (via lookup)
...
- ✅ `catalog_entity_id` → Link to merchant/service in global catalog
- ✅ `tags`, `entity_kind`, `entity_id`
- ✅ `created_at`
- ✅ `updated_at`
- `account_id`: uuid (SSOT for balance is ledger_entries)
- `to_account_id`: uuid (for internal transfers)
- `catalog_entity_id`: uuid (Lookup for merchant identity: name, logo)
...
Auto-created → transactions table (for opening balance)
├─ account_id
├─ type = 'income'
├─ amount = opening_balance
└─ description = 'Opening Balance'
(Note: This triggers a ledger entry via DB triggers)
```

**Columns Used:**
- ✅ `id`, `user_id`, `name`, `type`, `currency_code`
- ✅ `opening_balance`, `cached_balance`, `min_buffer`
...
**Columns Used:**
- ✅ `id`, `user_id`, `name`, `type` [UNIQUE (user_id, lower(trim(name)), type)]
- ✅ `catalog_entity_id` → Link to global taxonomy
- ✅ `is_system` → Reset-proof flag
- ✅ `created_at`
...
├─ catalog_entity_id → Linked via `catalogService.resolveEntity` normalization flow
...
- **Mapping Flow**:
  - Raw Input (e.g., "NET FLIX") -> `normalizeForMatch()` -> "netflix".
  - Check `user_catalog_link` for existing user mapping.
  - If no link, check `catalog_entities` for global `normalized_name` match.
  - If match found, use `catalog_entity_id` and potentially its `default_category_id`.
  - Save result in `user_catalog_link` to persist the mapping for the user.
...
├─ catalog_entity_id → Link to global template/pattern
└─ created_at
```

- ✅ `smart_suggestions` - Documented in database-structure.md
...
- ✅ `fee_rules` - Documented in database-structure.md
...
**API Method:** Managed by `on_ledger_entry_change` trigger

### Accounts & Balances
- `accounts.cached_balance` is a **derived cache**.
- **TRUTH:** Sum of `ledger_entries` for that `account_id`.
- **Enforcement:** `on_ledger_entry_change` trigger updates `cached_balance` on every insert/update/delete in the ledger.
- ✅ **System Balancing:** Income/Expense use a `System: Balancing` account for the offset entry.
- ✅ **Direction:** Standardized to `IN` / `OUT`.
- ✅ **Enforcement:** `validate_ledger_balance` trigger enforces `Sum(IN) = Sum(OUT)` per transaction.
