...
import { CircleDot, ShieldCheck, Zap, TrendingUp, AlertTriangle, ShieldAlert, TrendingDown, ChevronDown, Info, Bot } from 'lucide-react';
...
export const AdvancedInsights: React.FC<AdvancedInsightsProps> = ({
    ...
    const analysis = useMemo(() => analyzeFinancialFreedom({
        ...
        userName: userName,
        currency: currency,
        currentMonthExpenses: expenses.filter(e => {
            const d = new Date(e.date);
            const today = new Date();
            return d.getMonth() === today.getMonth() && d.getFullYear() === today.getFullYear();
        })
    }), [expenses, incomes, accounts, investments, liabilities, goals, debts, savingsRate, healthScore, userName, currency]);
    ...
    return (
        <div className="col-span-full mt-12 mb-4">
            ...
            <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className={`col-span-full mb-8 segmented-stack relative overflow-hidden ${analysis.alertColor === '#730800' ? 'garnet-alert border-2 shadow-2xl shadow-red-900/20 text-red-100' : 'bg-[#0F172A] border border-white/5'}`}
            >
                {analysis.alertColor === '#730800' ? (
                    ...
                    <MeshBackground variant="invest" className="opacity-[0.1]" />
                )}
                <div className="stack-cap flex justify-between items-center relative z-10">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[9px] text-slate-500 font-bold uppercase tracking-wider">Your Best Choice</p>
                        </div>
                    </div>
                    <div className="text-right">
                        <p className="text-[8px] text-slate-500 uppercase font-black mb-1">Plan Status</p>
                        <span
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-4">
                            <div className="flex justify-between items-end">
                                <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">The 80/20 Rule</span>
                                <span className="text-[10px] font-bold text-blue-400">Smart Balance</span>
                            </div>
                            ...
                            {analysis.realReturn && (
                                <div className="mt-4 p-3 bg-emerald-500/5 border border-emerald-500/10 rounded-xl flex items-center justify-between group overflow-hidden relative">
                                    ...
                                    <div>
                                        <p className="text-[8px] text-emerald-500/60 uppercase font-black mb-0.5 tracking-widest">True Growth</p>
                        ...
                        >
                            ...
                            <div className="mt-4 flex items-start gap-3 p-3 bg-blue-500/5 border border-blue-500/10 rounded-lg">
                                ...
                                <p className="text-[11px] text-slate-300 font-medium italic">
                                    "Our Recommendation: {analysis.tradeOff.comparisonMessage}"
                                </p>
                            </div>
                        </motion.div>
                    )}
                    ...
                    {analysis.pivotActions && analysis.pivotActions.length > 0 && (
                        <div className="mt-8 pt-8 border-t border-white/5">
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">What to do now</span>
                            </div>
            ...
            <div className="flex flex-col items-center gap-6 mb-10">
                ...
                <Button
                    ...
                    variant="ghost"
                    className="group relative px-8 h-12 bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:bg-white/10 transition-all"
                >
                    ...
                    <div className="flex items-center gap-3 relative z-10">
                        <TrendingUp className={`w-4 h-4 text-blue-400 transition-transform duration-500 ${showDetails ? 'rotate-90' : ''}`} />
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-100">
                            {showDetails ? 'Hide Detailed Analytics' : 'See Detailed Analytics'}
                        </span>
                        <ChevronDown className={`w-3 h-3 text-slate-500 transition-transform duration-500 ${showDetails ? 'rotate-180' : ''}`} />
                    </div>
                </Button>
            </div>
                    ...
                    >
                        ...
                        {analysis.summarySentence && (
                            <motion.div
                                ...
                                className="p-8 rounded-[32px] bg-slate-900/50 border border-white/5 relative overflow-hidden group"
                            >
                                <MeshBackground variant="ghost" className="opacity-[0.1]" />
                                <div className="absolute top-0 right-0 w-64 h-64 bg-slate-400/5 blur-[80px] -mr-32 -mt-32" />
                                <div className="relative z-10 flex items-start gap-4">
                                    <div className="p-3 bg-slate-800/50 rounded-2xl border border-white/10">
                                    ...
                                    <div>
                                        ...
                                        <div className="mt-6 flex flex-wrap gap-4">
                                            <div className="flex items-center gap-2 px-3 py-1.5 bg-white/5 rounded-full border border-white/5">
                                                <div className={`w-1.5 h-1.5 rounded-full ${analysis.interpretation?.liquidityStatus === 'High' ? 'bg-[#0A84FF]' : 'bg-[#FF9F0A]'}`} />
                                                <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Safe Zone Status: {analysis.interpretation?.liquidityStatus}</span>
                                            </div>
                                            <div className="flex items-center gap-2 px-3 py-1.5 bg-white/5 rounded-full border border-white/5">
                                                <div className={`w-1.5 h-1.5 rounded-full ${analysis.interpretation?.securityStatus === 'Locked' ? 'bg-[#0A84FF]' : 'bg-[#FF9F0A]'}`} />
                                                <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Security Zone: {analysis.interpretation?.securityStatus}</span>
                                            </div>
                                            <div className="flex items-center gap-2 px-3 py-1.5 bg-white/5 rounded-full border border-white/5">
                                                <div className="w-1.5 h-1.5 rounded-full bg-[#BF5AF2]" />
                                                <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Growth Zone: {analysis.interpretation?.safeZoneValue}%</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            {/* 2. Spending Nodes (Donut) - ACTION ZONE */}
                            <div className="segmented-stack bg-slate-900 border border-white/5 relative overflow-hidden">
                                <MeshBackground variant="safe" animate />
                            ...
                            {/* 3. Spending Power Trend - ACTION ZONE */}
                            <div className="segmented-stack bg-slate-900 border border-white/5 relative overflow-hidden">
                                <MeshBackground variant="safe" animate />
                                <div className="absolute top-0 left-0 w-1 h-full bg-teal-500/50" />
                            ...
                            {/* 4. Income vs Spending (Bar) - ACTION ZONE */}
                            <div className="segmented-stack bg-slate-900 border border-white/5 relative overflow-hidden">
                                <MeshBackground variant="safe" animate />
                                <div className="absolute top-0 left-0 w-1 h-full bg-teal-500/50" />
                                <div className="stack-cap flex justify-between items-center border-white/5 relative z-10">
                            ...
                            {/* 5. The Risk Meter (DTI) - ACTION ZONE */}
                            <div className="segmented-stack bg-slate-900 border border-white/5 col-span-full relative min-h-[180px] overflow-hidden">
                                <MeshBackground variant="safe" animate />
                                <div className="absolute top-0 left-0 w-1 h-full bg-teal-500/50" />
                                {dtiRatio > 0 ? (
                                    <>
                                        <div className="stack-cap flex justify-between items-start border-white/5 relative z-10">
                                            <div className="flex flex-col gap-1.5">
                                                <h3 className="text-slate-500 text-[9px] font-bold uppercase tracking-[0.2em]">Action Zone: Risk Leak</h3>
                                                <div className="text-4xl font-black text-white font-numeric leading-none">{(dtiRatio * 100).toFixed(1)}%<span className="text-sm text-slate-500 font-normal ml-2 font-sans truncate">Debt-to-Income</span></div>
                                            </div>
                            ...
                            {/* 6. The Leakage Tracker (Outflow) - ACTION ZONE */}
                            <div className="segmented-stack bg-slate-900 border border-white/5 col-span-full relative min-h-[180px] overflow-hidden">
                                <MeshBackground variant="debt" className="opacity-[0.1]" />
                                <div className="absolute top-0 left-0 w-1 h-full bg-orange-500/50" />
                                {(totalIncome > 1 || totalSpent > 0) ? (
                                    <>
                                        <div className="stack-cap flex justify-between items-start border-white/5 relative z-10">
                                            <div className="flex flex-col gap-1.5">
                                                <h3 className="text-slate-500 text-[9px] font-bold uppercase tracking-[0.2em]">Action Zone: Leakage Tracking</h3>
                                                <div className="text-4xl font-black text-white font-numeric leading-none">{(outflowRatio * 100).toFixed(1)}%<span className="text-sm text-slate-500 font-normal ml-2 font-sans truncate">Total Outflow</span></div>
                                            </div>
                                                ...
                                                <div className="space-y-6">
                                                    <div className="grid grid-cols-1 min-[400px]:grid-cols-2 gap-4">
                                                        ...
                                                        <div className="p-4 bg-slate-900 border border-white/5 rounded-xl">
                                                            ...
                                                            <span className="text-sm font-bold text-rose-400 font-numeric">{formatFinancialValue(totalSpent, currency)}</span>
                                                        </div>
                                                    </div>
